...
import { BiTime, BiDetail, BiLayer, BiMapPin, BiUser } from "react-icons/bi";
    ...
    <div className="bg-white rounded-[40px] shadow-bento border border-emerald-50 overflow-hidden">
      <div className="p-10 border-b border-emerald-50 flex items-center justify-between">
        <div>
          ...
          <p className="text-emerald-800/40 text-[10px] font-black uppercase tracking-widest mt-1 italic">Mission Success Ledger</p>
        </div>
        <div className="hidden sm:block">
          <span className="bg-emerald-50 text-emerald-700 px-4 py-2 rounded-xl text-xs font-bold border border-emerald-100 italic">
            {claimedDonations.length} Operations Completed
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-10">
            ...
            <div className="p-7">
              <div className="flex flex-wrap items-center gap-2 mb-4">
                <span className="bg-emerald-100 text-emerald-800 text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-widest flex items-center gap-1">
                  <BiLayer /> {donation.foodCategory || "General"}
                </span>
                <span className="text-[10px] font-bold text-emerald-900/40 flex items-center gap-1">
                  <BiTime /> {donation.claimedAt ? new Date(donation.claimedAt).toLocaleDateString() : "Historical"}
                </span>
              </div>
              ...
              <div className="space-y-3 mb-8 border-t border-emerald-50 pt-6">
                <div className="flex items-center gap-3">
                  <BiUser className="text-primary text-lg" />
                  <p className="text-xs font-bold text-emerald-900">Donor: {donation.donorName}</p>
                </div>
                <div className="flex items-start gap-3">
                  <BiMapPin className="text-primary text-lg mt-0.5" />
                  <p className="text-xs font-bold text-emerald-800/60 leading-tight">{donation.donorAddress}</p>
                </div>
              </div>
              ...
              <Link to={`/donation/${btoa(donation._id)}`} className="block">
                <button className="w-full flex items-center justify-center gap-2 bg-white text-emerald-900 font-black py-4 rounded-2xl border border-emerald-100 hover:bg-emerald-900 hover:text-white hover:border-emerald-900 transition-all text-xs uppercase tracking-widest">
                  <BiDetail className="text-lg" /> Audit Metadata
                </button>
              </Link>
            </div>
          </div>
        ...
        {claimedDonations.length === 0 && (
          <div className="col-span-full py-20 text-center text-emerald-900/20 italic font-black uppercase tracking-widest">
            No missions found in impact ledger.
          </div>
        )}
      </div>
    </div>
